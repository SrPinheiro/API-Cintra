package com.cintra.controller.doctor;
...
import com.cintra.services.DoctorService;
import com.cintra.services.EspecialidadeService;
...
public class DoctorController {
    ...
    public Object FindAll(){
        try {
            if (currentUser != null && currentUser.getRole().name("admin")) {
                List<UserDTO> users = doctorService.findAll().stream().map(u -> (Doctor) u).collect(Collectors.toList());
    ...
    public Object findEspecialidades(){
        return new ResponseEntity<>(especialidadeService.findAll(), HttpStatus.OK);
    }

    @PostMapping()
    public Object createDoctor(HttpServletResponse response, @RequestBody Doctor u){
        try{
            if(!currentUser.getRole().equals(roleService.findByName("admin")))
        ...
        u.setRole(roleService.findByName("doctor"));
    
        return new ResponseEntity<>(doctorService.save(u), HttpStatus.CREATED);
    }
